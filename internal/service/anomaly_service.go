...
func (s *AnomalyService) Patrol(ctx context.Context) (int, error) {
	processed := 0
	pending, err := s.d.Repo.Samples.ListUnprocessed(ctx, 500)
	if err != nil {
		return 0, err
	}
	earlyIDs := make([]int64, 0, len(pending))
	for _, sample := range pending {
		arts, listErr := s.d.Repo.Artifacts.ListByUnit(ctx, sample.StorageUnitID)
		levelIDs := make([]int64, 0, len(arts))
		for _, artifact := range arts { levelIDs = append(levelIDs, artifact.LevelID) }
		if _, ruleErr := s.d.strictestRuleForLevels(ctx, s.d.Repo, unique(levelIDs)); listErr == nil && ruleErr != nil {
			earlyIDs = append(earlyIDs, sample.ID)
		}
	}
	if err := s.d.Repo.Samples.MarkProcessed(ctx, earlyIDs); err != nil { return 0, err }
